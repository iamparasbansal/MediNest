...
doctors = pd.read_csv("datasets/doctor_data.csv")
...
def helper(dis):
    ...
    dctr = doctors[doctors['disease'] == dis]['doctor']

    return desc, pre, med, die, wrkout, dctr
...
def get_predicted_value(patient_symptoms):
    ...
    if missing_symptoms:
        # Handle missing symptoms (e.g., return an error message)
        error_message = f"The following symptoms are not recognized: {', '.join(missing_symptoms)}"
...
def home():
    if request.method == 'POST':
        ...
        if symptoms == "Symptoms":
            ...
            if predicted_disease.startswith("The following symptoms are not recognized:"):
                # Handle error message
                message = predicted_disease
                return render_template('index.html', message=message)
            else:
                dis_des, precautions, medications, rec_diet, workout, my_doctor = helper(predicted_disease)
                ...
                return render_template('index.html', predicted_disease=predicted_disease,
                                       ...
                                       workout=workout, doctor=my_doctor)
